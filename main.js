...
var $author$project$Main$Model = F7(
	function (year, publicHolidays, weekendDays, longWeekends, numberOfForcedLeaves, showNonWeekendsToo, startOfWeek) {
		return {longWeekends: longWeekends, numberOfForcedLeaves: numberOfForcedLeaves, publicHolidays: publicHolidays, showNonWeekendsToo: showNonWeekendsToo, startOfWeek: startOfWeek, weekendDays: weekendDays, year: year};
	});
...
var $author$project$Main$changeYear = F2(
	function (year, date) {
		return A3(
...
var $author$project$CalendarGenerator$stringToWeekday = function (weekday) {
	switch (weekday) {
		case 'Mon':
...
var $author$project$Main$init = function (_v0) {
	var year = _v0.year;
	var initPublicHolidays = _v0.initPublicHolidays;
	var numberOfForcedLeaves = _v0.numberOfForcedLeaves;
	var startOfWeek = _v0.startOfWeek;
	var weekendDays = _v0.weekendDays;
	return _Utils_Tuple2(
		A7(
			...
			A2(
				$elm$core$List$map,
				$author$project$Main$changeYear(year),
				A2(
					$elm$core$List$filterMap,
					A2($elm$core$Basics$composeR, $justinmimbs$date$Date$fromIsoString, $elm$core$Result$toMaybe),
					initPublicHolidays)),
			A2($elm$core$List$map, $author$project$CalendarGenerator$stringToWeekday, weekendDays),
			_List_Nil,
			numberOfForcedLeaves,
			true,
			$author$project$CalendarGenerator$stringToWeekday(startOfWeek)),
		A2(
			$elm$core$Task$perform,
			function (_v1) {
...
var $author$project$CalendarDays$getItemWithMaxLength = A2(
	$elm$core$Basics$composeL,
	$elm$core$Maybe$withDefault(_List_Nil),
	$elm_community$list_extra$List$Extra$maximumBy($elm$core$List$length));
				...
				while (true) {
					...
					if (prevDayIsHoliday) {
						var $temp$result = newResult,
							...
							$temp$days_ = days_;
						result = $temp$result;
						pointDate = $temp$pointDate;
						days_ = $temp$days_;
						continue recTraverseBack;
					} else {
						if (!days_) {
							return newResult;
						} else {
...
var $author$project$CalendarDays$traverseForwards = F4(
	function (publicHolidays, weekends, fromDate, days) {
		...
		return A3(recTraverseForward, _List_Nil, fromDate, days);
	});
				...
				while (true) {
					...
					if (backwards < 0) {
						...
						var $temp$result = A2(
							$elm$core$List$cons,
							sortedUnique(
								_Utils_Tuple2(backwards, forwards)),
							result),
							$temp$date_ = date_,
							$temp$_v0 = _Utils_Tuple2(backwards - 1, forwards + 1);
						result = $temp$result;
						date_ = $temp$date_;
						_v0 = $temp$_v0;
						continue recGetLeaveRangeFromDate;
					}
				}
			});
...
var $author$project$Main$saveNumberOfForcedLeavesToStorage = _Platform_outgoingPort('saveNumberOfForcedLeavesToStorage', $elm$json$Json$Encode$string);
var $author$project$Main$savePhToStorage = _Platform_outgoingPort('savePhToStorage', $elm$json$Json$Encode$string);
var $author$project$Main$saveStartOfWeekToStorage = _Platform_outgoingPort('saveStartOfWeekToStorage', $elm$json$Json$Encode$string);
var $author$project$Main$saveWeekendDaysToStorage = _Platform_outgoingPort('saveWeekendDaysToStorage', $elm$json$Json$Encode$string);
...
var $author$project$CalendarGenerator$weekdayToTripleCharacterString = function (weekday) {
	switch (weekday.$) {
		case 'Mon':
			return 'Mon';
		case 'Tue':
			...
			switch (msg.$) {
				...
				case 'UpdateYear':
					...
					var year = A3(
						...
						$elm$core$Maybe$withDefault(2021),
						str);
					var newPh = A2(
						$elm$core$List$map,
						$author$project$Main$changeYear(year),
				...
				case 'GenerateLongWeekends':
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								longWeekends: $elm_community$list_extra$List$Extra$unique(lwdFilteredForWeekends)
							}),
						$elm$core$Platform$Cmd$batch(
							_List_fromArray(
								[
									$author$project$Main$savePhToStorage(
									A2(
										...
										A2(
											...
											A2($elm$core$List$map, $justinmimbs$date$Date$toIsoString, model.publicHolidays)))),
									$author$project$Main$saveNumberOfForcedLeavesToStorage(
									A2(
										$elm$json$Json$Encode$encode,
										0,
										$elm$json$Json$Encode$int(model.numberOfForcedLeaves))),
									$author$project$Main$saveWeekendDaysToStorage(
									A2(
										$elm$json$Json$Encode$encode,
										0,
										A2(
											$elm$json$Json$Encode$list,
											$elm$json$Json$Encode$string,
											A2($elm$core$List$map, $author$project$CalendarGenerator$weekdayToTripleCharacterString, model.weekendDays))))
								])));
				case 'ToggleWeekendDays':
					var weekday = msg.a;
					var checked = msg.b;
					var newWeekendDays = checked ? A2($elm$core$List$cons, weekday, model.weekendDays) : A2(
						$elm$core$List$filter,
						$elm$core$Basics$neq(weekday),
						model.weekendDays);
					var $temp$msg = $author$project$Main$GenerateLongWeekends,
						$temp$model = _Utils_update(
						model,
					...
					model = $temp$model;
					continue update;
				case 'UpdateStartOfWeek':
					var startOfWeek = msg.a;
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{startOfWeek: startOfWeek}),
						$author$project$Main$saveStartOfWeekToStorage(
							A2(
								$elm$json$Json$Encode$encode,
								0,
								$elm$json$Json$Encode$string(
									$author$project$CalendarGenerator$weekdayToTripleCharacterString(startOfWeek)))));
				default:
					var cmsg = msg.a;
					var date = cmsg.a;
					if (date.$ === 'Nothing') {
...
var $author$project$Main$viewLegend = function (_v0) {
	var phs = _v0.phs;
	var lwds = _v0.lwds;
	var weekends = _v0.weekends;
	var timeOffDays = A2(
		$elm$core$List$filter,
		function (date) {
			return A2(
				$elm$core$List$all,
				$elm$core$Basics$neq(date),
				phs);
		},
		A2(
			$elm$core$List$filter,
				...
				_List_fromArray(
					[
						...
						_Utils_ap(
							...
							_List_fromArray(
								[
									$elm$html$Html$Attributes$class('border border-green-200 bg-green-200 inline-block')
								])),
						_List_Nil),
						A2(
						$elm$html$Html$span,
						...
						_List_fromArray(
							[
								$elm$html$Html$text(
								'Potential long leave (' + ($elm$core$String$fromInt(
									$elm$core$List$length(lwds)) + ')'))
							]))
					])),
				A2(
				$elm$html$Html$div,
				...
				_List_fromArray(
					[
						...
						_Utils_ap(
							...
							_List_fromArray(
								[
									$elm$html$Html$Attributes$class('border border-green-500 inline-block')
								])),
						_List_Nil),
						A2(
						$elm$html$Html$span,
						...
						_List_fromArray(
							[
								$elm$html$Html$text(
								'Day marked as public holiday (' + ($elm$core$String$fromInt(
									$elm$core$List$length(phs)) + ')'))
							]))
					])),
				A2(
				$elm$html$Html$div,
				...
				_List_fromArray(
					[
						...
						_Utils_ap(
							...
							_List_fromArray(
								[
									$elm$html$Html$Attributes$class('border border-red-100 bg-red-100 inline-block')
								])),
						_List_Nil),
						A2(
						$elm$html$Html$span,
						...
						_List_fromArray(
							[
								$elm$html$Html$text(
								'Suggested time-off days (' + ($elm$core$String$fromInt(
									$elm$core$List$length(timeOffDays)) + ')'))
							]))
					]))
			]));
};
var $author$project$Main$UpdateStartOfWeek = function (a) {
	return {$: 'UpdateStartOfWeek', a: a};
};
...
var $author$project$Main$weekdays = _List_fromArray(
...
var $author$project$Main$viewWeekStartSelector = function (selectedWeekday) {
	var option = function (weekday) {
		return A2(
			$elm$html$Html$option,
			_List_fromArray(
				[
					$elm$html$Html$Attributes$selected(
					_Utils_eq(weekday, selectedWeekday)),
					$elm$html$Html$Attributes$value(
					$author$project$CalendarGenerator$weekdayToTripleCharacterString(weekday)),
					$elm$html$Html$Events$onClick(
					$author$project$Main$UpdateStartOfWeek(weekday))
				]),
			_List_fromArray(
				[
					$elm$html$Html$text(
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				_List_Nil,
				A2($elm$core$List$map, option, $author$project$Main$weekdays))
			]));
};
var $author$project$Main$ToggleWeekendDays = F2(
	function (a, b) {
		return {$: 'ToggleWeekendDays', a: a, b: b};
	});
var $author$project$CalendarGenerator$weekdayToSingleCharacter = function (weekday) {
	switch (weekday.$) {
		case 'Mon':
			return 'M';
		case 'Tue':
...
var $author$project$Main$viewWeekendSelector = function (weekends) {
	var renderCheckbox = F2(
		function (selectedWeekends, weekday) {
			return A2(
				$elm$html$Html$span,
				_List_fromArray(
					[
						$elm$html$Html$Attributes$class('inline-block mr-3')
					]),
				_List_fromArray(
					[
						A2(
						...
						_List_fromArray(
							[
								...
								'checkbox-' + $author$project$CalendarGenerator$weekdayToSingleCharacter(weekday)),
								$elm$html$Html$Attributes$checked(
								A2($elm$core$List$member, weekday, selectedWeekends)),
								$elm$html$Html$Events$onCheck(
								$author$project$Main$ToggleWeekendDays(weekday))
							]),
						_List_Nil),
						A2(
						$elm$html$Html$label,
						...
						_List_fromArray(
							[
								$elm$html$Html$text(
								$author$project$CalendarGenerator$weekdayToSingleCharacter(weekday))
							]))
					]));
		});
	return A2(
...
var $author$project$CalendarGenerator$timeWeekdayToDateInterval = function (weekday) {
	switch (weekday.$) {
		case 'Sun':
...
var $author$project$CalendarGenerator$generateMonthList = F3(
	function (month, year, startOfWeek) {
		var startDateOfMonth = A3($justinmimbs$date$Date$fromCalendarDate, year, month, 1);
		var padLeftAmount = A3(
			$justinmimbs$date$Date$diff,
			$justinmimbs$date$Date$Days,
			A2(
				$justinmimbs$date$Date$floor,
				$author$project$CalendarGenerator$timeWeekdayToDateInterval(startOfWeek),
				startDateOfMonth),
			startDateOfMonth);
		var endOfWeek = $author$project$CalendarGenerator$getEndOfWeek(startOfWeek);
		var endDateOfMonth = A3(
			$justinmimbs$date$Date$add,
			$justinmimbs$date$Date$Days,
			-1,
			A3($justinmimbs$date$Date$add, $justinmimbs$date$Date$Months, 1, startDateOfMonth));
		var padRightAmount = A3(
			$justinmimbs$date$Date$diff,
			$justinmimbs$date$Date$Days,
			endDateOfMonth,
			A2(
				$justinmimbs$date$Date$ceiling,
				$author$project$CalendarGenerator$timeWeekdayToDateInterval(endOfWeek),
				endDateOfMonth));
		var datesAsListofMaybes = _Utils_ap(
			A2($elm$core$List$repeat, padLeftAmount, $elm$core$Maybe$Nothing),
			_Utils_ap(
				A2(
					$elm$core$List$map,
					$elm$core$Maybe$Just,
					A4(
						...
						1,
						startDateOfMonth,
						A3($justinmimbs$date$Date$add, $justinmimbs$date$Date$Days, 1, endDateOfMonth))),
				A2($elm$core$List$repeat, padRightAmount, $elm$core$Maybe$Nothing)));
		var result = A2($elm_community$list_extra$List$Extra$groupsOf, 7, datesAsListofMaybes);
		return result;
	});
	...
	function (month, year) {
		return A2(
			...
			_List_fromArray(
				[
					$elm$html$Html$text(
					$author$project$CalendarGenerator$monthToString(month) + (' ' + $elm$core$String$fromInt(year)))
				]));
	});
var $author$project$CalendarGenerator$ClickedDate = function (a) {
	return {$: 'ClickedDate', a: a};
};
...
var $author$project$CalendarGenerator$viewDate = F2(
	function (_v0, maybeDate) {
		var phs = _v0.phs;
		var lwds = _v0.lwds;
		var weekends = _v0.weekends;
		var isWeekend = A2(
			...
			A2(
				...
				function (weekday) {
					return A2(
						$elm$core$List$any,
						$elm$core$Basics$eq(weekday),
						weekends);
				},
				A2($elm$core$Maybe$map, $justinmimbs$date$Date$weekday, maybeDate)));
		var isPh = A2(
			$elm$core$Maybe$withDefault,
			false,
			A2(
		...
		var isLwDate = A2(
			...
			A2(
				...
				maybeDate));
		var isTimeOff = isLwDate && ((!isPh) && (!isWeekend));
		...
		var className = function () {
			var _v1 = _Utils_Tuple3(isPh, isLwDate, isTimeOff);
			if (_v1.a) {
		...
		return A2(
			...
			_Utils_ap(
				...
				_List_fromArray(
					[
						$elm$html$Html$Attributes$class('date border border-transparent cursor-pointer'),
...
var $author$project$CalendarGenerator$viewWeek = F2(
	function (daysData, week) {
		return A2(
			$elm$html$Html$div,
			$author$project$CalendarGenerator$weekDivStyle,
			A2(
				$elm$core$List$map,
				$author$project$CalendarGenerator$viewDate(daysData),
				week));
	});
var $author$project$CalendarGenerator$viewWeekHeader = function (startOfWeek) {
	...
	var toSpan = A2($elm$core$Basics$composeR, $author$project$CalendarGenerator$weekdayToSingleCharacter, span);
	return A2(
		$elm$html$Html$div,
		$author$project$CalendarGenerator$weekDivStyle,
		A2(
...
var $author$project$CalendarGenerator$viewMonth = F4(
	function (daysData, month, startOfWeek, year) {
		...
		return A2(
			...
			_Utils_ap(
				...
				A2(
					$elm$core$List$map,
					$author$project$CalendarGenerator$viewWeek(daysData),
					listOfWeeks)));
	});
var $author$project$Main$viewYear = F3(
	function (year, daysData, startOfWeek) {
		return A2(
			$elm$html$Html$div,
			_List_fromArray(
				[
			...
			A2(
				...
				function (month) {
					return A2(
						$elm$html$Html$map,
						$author$project$Main$CMsg,
						A4($author$project$CalendarGenerator$viewMonth, daysData, month, startOfWeek, year));
				},
	...
	return A2(
		...
		_List_fromArray(
			[
				A2($elm$html$Html$Attributes$style, 'display', 'grid'),
				A2($elm$html$Html$Attributes$style, 'grid-auto-flow', 'column'),
				A2($elm$html$Html$Attributes$style, 'grid-template-columns', '1fr 3fr'),
		...
		_List_fromArray(
			[
							...
							[
								...
								_List_fromArray(
									[
										...
										_List_fromArray(
											[
												$elm$html$Html$text('Click on a date to mark (or remove) it as a public holiday. By default, it shows some public holidays for India.')
											]))
									])),
								A2(
								$elm$html$Html$div,
								...
								_List_fromArray(
									[
										...
										_List_fromArray(
											[
												$elm$html$Html$text('Number of time-offs you are willing to take*')
											])),
										A2(
										$elm$html$Html$input,
										_List_fromArray(
								...
								_List_fromArray(
									[
										...
										_List_fromArray(
											[
												$elm$html$Html$text('Weekends')
											])),
										$author$project$Main$viewWeekendSelector(model.weekendDays)
									])),
								A2(
								$elm$html$Html$div,
								_List_Nil,
								_List_fromArray(
									[
										...
										_List_fromArray(
											[
												$elm$html$Html$text('Week starts on')
											])),
										$author$project$Main$viewWeekStartSelector(model.startOfWeek)
									])),
								A2(
								$elm$html$Html$div,
								_List_Nil,
								_List_fromArray(
									[
										...
										{
											lwds: $elm$core$List$concat(model.longWeekends),
				...
				_List_fromArray(
					[
						$elm$html$Html$Attributes$class('p-10 border-l')
					]),
				_List_fromArray(
					[
						A3(
				...
				function (weekendDays) {
					return A2(
						...
						function (startOfWeek) {
							return A2(
								$elm$json$Json$Decode$andThen,
								function (numberOfForcedLeaves) {
									return A2(
										$elm$json$Json$Decode$andThen,
										function (initPublicHolidays) {
											return $elm$json$Json$Decode$succeed(
												{initPublicHolidays: initPublicHolidays, numberOfForcedLeaves: numberOfForcedLeaves, startOfWeek: startOfWeek, weekendDays: weekendDays, year: year});
										},
										A2(
								...
								A2($elm$json$Json$Decode$field, 'numberOfForcedLeaves', $elm$json$Json$Decode$int));
						},
						A2($elm$json$Json$Decode$field, 'startOfWeek', $elm$json$Json$Decode$string));
				},
				A2(
					$elm$json$Json$Decode$field,
					'weekendDays',
